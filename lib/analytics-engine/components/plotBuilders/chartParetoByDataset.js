...
// DEPOIS
import { buildModelColorMarkerMaps, computeRowYLim, buildCellTraces } from "./paretoGrid.js";
...
export function buildChartParetoByDatasetPlots(paretoByDataset) {
  ...
  for (const [dataset, rows] of Object.entries(paretoByDataset)) {
    ...
    const data = [];
    const layout = {
      title: { text: `Accuracy vs. Inference Time for ${dataset} Dataset`, font: { size: 18 } },
      plot_bgcolor: "#ffffff",
    ...
    devices.forEach((device, colIdx) => {
      ...
      layout[`yaxis${axisSuffix}`] = { range: yLim, title: colIdx === 0 ? "Accuracy (%)" : "", tickformat: ",.0%", gridcolor: "#e5e7eb" };
