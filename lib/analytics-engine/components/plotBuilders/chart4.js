import { MOBILE_LEGEND_FONT_SIZE, colorForGroup } from "../chartTheme.js";
...
export function buildChart4Plot(chart4Stats) {
  ...
  const data = [];
  const layout = {
    title: { text: "Inference time by modelo, device and experiment", font: { size: 18 } },
    plot_bgcolor: "#ffffff",
  ...
  devices.forEach((device, i) => {
    ...
    layout[`xaxis${axisSuffix}`] = { domain: [domainStart, domainStart + domainWidth], title: "Model", tickangle: -45 };
    layout[`yaxis${axisSuffix}`] = { title: i === 0 ? "Inference time (ms)" : "", gridcolor: "#e5e7eb", rangemode: "tozero" };
    ...
    grupos.forEach((grupo, gi) => {
      ...
      modelOrder.forEach((modelo) => {
        ...
        if (!r) return;
        const time = 1000 / r.ips_mean;
        const timeLow = r.ips_ci_upper != null ? 1000 / r.ips_ci_upper : time;  // IPS maior → tempo menor
        const timeHigh = r.ips_ci_lower != null ? 1000 / r.ips_ci_lower : time; // IPS menor → tempo maior
        xs.push(modelo);
        ys.push(time);
        errPlus.push(timeHigh - time);
        errMinus.push(time - timeLow);
      });
      ...
      data.push({
        ...
        marker: { color: colorForGroup(grupo) },
        xaxis: `x${axisSuffix}`,
        yaxis: `y${axisSuffix}`,
        showlegend: true,
        hovertemplate: `<b>${grupo}</b><br>%{x}<br>Tempo: %{y:.2f} ms<extra></extra>`,
      });
    });
